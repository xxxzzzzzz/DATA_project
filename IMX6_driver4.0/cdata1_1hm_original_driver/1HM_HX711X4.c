...
static int weight_open (struct inode *inode, struct file *file){
  int ret;
      ...
      ret = gpio_request_one(GPIO5_14,GPIOF_DIR_IN, "fec-phy-reset");

      if(ret){printk("hx1 data gpio GPIO5_14 request failed\n");}  
      else {printk("hx1 data gpio  GPIO5_14  request success\n");}

      ret = gpio_request_one(GPIO5_15,GPIOF_OUT_INIT_LOW, "fec-phy-reset");
      if(ret){printk("hx1 clk gpio GPIO5_15 request failed\n");}
      else {printk("hx1 clk gpio GPIO5_15  request success\n");} 
...
static int weight2_open (struct inode *inode, struct file *file){
  int ret;
    ...
    if(!atomic_dec_and_test(&weightfd2)){
     ...
     ret = gpio_request_one(GPIO5_16,GPIOF_DIR_IN, "fec-phy-reset");
     if(ret){printk("hx2 data gpio GPIO5_16 request failed\n");}  
     else {printk("hx2 data gpio GPIO5_16  request success\n");}

     ret = gpio_request_one(GPIO5_17,GPIOF_OUT_INIT_LOW, "fec-phy-reset");
     if(ret){printk("hx2 clk gpio GPIO5_17 request failed\n");}
     else {printk("hx2 clk gpio GPIO5_17  request success\n");}
...
static int weight3_open (struct inode *inode, struct file *file){
  int ret;
     ...
     if(ret){printk("hx3 data gpio GPIO1_8 request failed\n");}  
     else {printk("hx3 data gpio GPIO1_8  request success\n");}

     ret = gpio_request_one(GPIO4_11,GPIOF_OUT_INIT_LOW, "fec-phy-reset");
     if(ret){printk("hx3 clk gpio GPIO4_11 request failed\n");}
     else {printk("hx3 clk gpio GPIO4_11  request success\n");}
...
static int weight4_open (struct inode *inode, struct file *file){
  int ret;
    ...
    if(!atomic_dec_and_test(&weightfd4)){
     ...
     if(ret){printk("hx4 data gpio GPIO4_14 request failed\n");}
     else {printk("hx4 data gpio GPIO4_14  request success\n");}

     ret = gpio_request_one(GPIO4_15,GPIOF_OUT_INIT_LOW, "fec-phy-reset");
     if(ret){printk("hx4 clk gpio GPIO4_15 request failed\n");}
     else {printk("hx4 clk gpio GPIO4_15  request success\n");}
