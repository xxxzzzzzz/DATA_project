...
void RX8025_Time_set(struct rtc_time *tm,struct rx8025_GPIO_definit* plat_gpio_data)
{
	...
	rtc_str[0] = bcd8421[tm->tm_sec];
	rtc_str[1] = bcd8421[tm->tm_min];
	rtc_str[2] = bcd8421[tm->tm_hour];
	rtc_str[3] = bcd8421[tm->tm_wday];
	rtc_str[4] = bcd8421[tm->tm_mday];
	rtc_str[5] = bcd8421[tm->tm_mon];
	rtc_str[6] = bcd8421[tm->tm_year];
...
void RX8025_get_Time(struct rtc_time *tm,struct rx8025_GPIO_definit* plat_gpio_data)
{
	...
	tm->tm_sec = (unsigned int)bcd2bin(rtc_str[0]);
	tm->tm_min = (unsigned int)bcd2bin(rtc_str[1]);
	tm->tm_hour = (unsigned int)bcd2bin(rtc_str[2]);
	tm->tm_wday = (unsigned int)bcd2bin(rtc_str[3]);
	tm->tm_mday = (unsigned int)bcd2bin(rtc_str[4]);
	tm->tm_mon = (unsigned int)bcd2bin(rtc_str[5]);
	tm->tm_year = (unsigned int)bcd2bin(rtc_str[6]);
...
void RX8025_INTA_OutIrq_set(unsigned char setmin,unsigned char sethour,struct rx8025_GPIO_definit* plat_gpio_data)
{
	...
	printk("befor sermin = %d  serhour = %d\n",setmin,sethour);
	temp = R8025AC_Read_byte(CONTROL1,plat_gpio_data);

	R8025AC_Write(ALARM_D_MIN,bin2bcd(bcd8421[setmin]),plat_gpio_data);//set time out min
	R8025AC_Write(ALARM_D_HOUR,bin2bcd(bcd8421[sethour]),plat_gpio_data);//set time out hours
...
/*
 *query CONTROL2 reg  low 3 bit status
 * 
 */
int RX8025_Read_CONTROL2_CTFG(struct rx8025_GPIO_definit* plat_gpio_data)
{
	if(((R8025AC_Read_byte(CONTROL2,plat_gpio_data)&0x11)==0x11)||
		((R8025AC_Read_byte(CONTROL2,plat_gpio_data)&0X01)==0X01))
	{
...
/*reset CONTROL2 reg low 3 bit*/
void RX8025_SET_CONTROL2_CTFG(struct rx8025_GPIO_definit* plat_gpio_data)
{
	unsigned char tmp = 0,CTFG;

	tmp = R8025AC_Read_byte(CONTROL2,plat_gpio_data);
#ifdef DEBUG	
	printk("before 0x0f = 0x%x\n",tmp);
#endif
	if(((R8025AC_Read_byte(CONTROL2,plat_gpio_data)&0x11)==0x11)||
		((R8025AC_Read_byte(CONTROL2,plat_gpio_data)&0X01)==0X01))
	{
